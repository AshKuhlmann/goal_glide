...
def test_pomo_session_persisted(tmp_path, monkeypatch):
    ...
    runner.invoke(
        cli, ["goal", "pomo", "start", "--duration", "1", "--goal", gid],
        env={"GOAL_GLIDE_DB_DIR": str(tmp_path)},
    )
...
def test_goal_commands_invalid_id(cmd, goal_id, tmp_path):
    ...
    args = [cmd, goal_id]
    if cmd == "remove":
        result = runner.invoke(cli, ["goal", cmd, goal_id], input="y\n", env=env)
    else:
        result = runner.invoke(cli, ["goal", cmd, goal_id], env=env)
