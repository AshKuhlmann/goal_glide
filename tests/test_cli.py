...
def test_add_list_remove(tmp_path):
    ...
    result = runner.invoke(
        goal, ["add", "Test Goal"], env={"GOAL_GLIDE_DB_DIR": str(tmp_path)}
    )
...
def test_remove_cancel_keeps_goal(tmp_path):
    ...
    runner.invoke(goal, ["add", "Stay"], env={"GOAL_GLIDE_DB_DIR": str(tmp_path)})
...
def test_pomo_session_persisted(tmp_path, monkeypatch):
    ...
    runner = CliRunner()
    res = runner.invoke(goal, ["add", "G"], env={"GOAL_GLIDE_DB_DIR": str(tmp_path)})
    ...
    runner.invoke(
        goal, ["pomo", "stop"], env={"GOAL_GLIDE_DB_DIR": str(tmp_path)}
    )
