...
def stats_cmd(
    ...
    for label, bar in bars:
        click.echo(f"{label} {bar}")

    streak = current_streak(storage, end)
    click.echo(f"\N{FIRE}  Current streak: {streak} days")

    longest = longest_streak(storage)
    click.echo(f"\N{HIGH VOLTAGE SIGN}  Longest streak: {longest} days")
    ...
    if mpd:
        ...
        click.echo(f"\N{CALENDAR}  Most productive day: {mpd} (avg. {avg_fmt})")
